...
#include "Cache.h"
#include "CudaFFmpegDeviceInterface.h"
...
static bool g_cuda_ffmpeg = register_device_interface(
    DeviceInterfaceKey(kStableCUDA, /*variant=*/"ffmpeg"),
    [](const StableDevice& device) {
      return new CudaFFmpegDeviceInterface(device);
    });
...
CudaFFmpegDeviceInterface::CudaFFmpegDeviceInterface(const StableDevice& device)
    : DeviceInterface(device) {
  STD_TORCH_CHECK(
      g_cuda_ffmpeg, "CudaFFmpegDeviceInterface was not registered!");
  STD_TORCH_CHECK(
...
CudaFFmpegDeviceInterface::~CudaFFmpegDeviceInterface() {
...
void CudaFFmpegDeviceInterface::initialize(
    const SharedAVCodecContext& codec_context) {
  codec_context_ = codec_context;
}

void CudaFFmpegDeviceInterface::initialize_video(
...
void CudaFFmpegDeviceInterface::register_hardware_device_with_codec(
...
UniqueAVFrame
CudaFFmpegDeviceInterface::maybe_convert_av_frame_to_nv12_or_rgb24(
...
void CudaFFmpegDeviceInterface::convert_av_frame_to_frame_output(
    UniqueAVFrame& av_frame,
...
// device
std::optional<const AVCodec*> CudaFFmpegDeviceInterface::find_codec(
...
std::string CudaFFmpegDeviceInterface::get_details() {
...
UniqueAVFrame
CudaFFmpegDeviceInterface::convert_tensor_to_av_frame_for_encoding(
...
void CudaFFmpegDeviceInterface::setup_hardware_frame_context_for_encoding(
    AVCodecContext* codec_context) {
  STD_TORCH_CHECK(codec_context != nullptr, "codecContext is null");
  STD_TORCH_CHECK(
      hardware_device_ctx_, "Hardware device context has not been initialized");
