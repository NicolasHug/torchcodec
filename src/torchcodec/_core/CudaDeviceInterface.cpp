...
pfn_sequence_callback(void* p_user_data, CUVIDEOFORMAT* video_format) {
  auto decoder = static_cast<CudaDeviceInterface*>(p_user_data);
...
void CudaDeviceInterface::initialize_bsf(
...
// etc.
int CudaDeviceInterface::stream_property_change(CUVIDEOFORMAT* video_format) {
...
// the NVCUVID parser.
int CudaDeviceInterface::send_packet(ReferenceAVPacket& packet) {
  if (cpu_fallback_) {
...
int CudaDeviceInterface::send_eof_packet() {
  if (cpu_fallback_) {
...
int CudaDeviceInterface::frame_ready_for_decoding(CUVIDPICPARAMS* pic_params) {
...
int CudaDeviceInterface::frame_ready_in_display_order(
    CUVIDPARSERDISPINFO* disp_info) {
...
int CudaDeviceInterface::receive_frame(UniqueAVFrame& av_frame) {
  if (cpu_fallback_) {
...
void CudaDeviceInterface::unmap_previous_frame() {
...
UniqueAVFrame CudaDeviceInterface::convert_cuda_frame_to_av_frame(
...
void CudaDeviceInterface::flush() {
  if (cpu_fallback_) {
    cpu_fallback_->flush();
    return;
  }
...
UniqueAVFrame CudaDeviceInterface::transfer_cpu_frame_to_gpu(
...
void CudaDeviceInterface::apply_rotation(
    FrameOutput& frame_output,
    std::optional<torch::stable::Tensor> pre_allocated_output_tensor) {
  int k = 0;
  switch (rotation_) {
...
OutputDtype CudaDeviceInterface::get_pre_allocation_dtype(
