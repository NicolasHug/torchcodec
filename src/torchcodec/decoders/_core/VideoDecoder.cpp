...
void VideoDecoder::createFilterGraph(
  ...
  ffmpegStatus = avfilter_graph_config(filterGraphContext.filterGraph.get(), nullptr);
  if (ffmpegStatus < 0) {
    throw std::runtime_error(
...
torch::Tensor VideoDecoder::convertAVFrameToTensorUsingFilterGraph(
    int streamIndex,
    const AVFrame* avFrame) {
  FilterGraphContext& filterGraphContext = streamInfos_[streamIndex].filterGraphContext;
  int ffmpegStatus =
  ...
  ffmpegStatus =
      av_buffersink_get_frame(filterGraphContext.sinkContext, filteredAVFrame.get());
